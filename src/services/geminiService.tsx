...
const addTransactionTool: FunctionDeclaration = {
  name: "addTransaction",
  description:
    "Add a new financial transaction (income or expense) to the budget tracker. ONLY call this if the user explicitly states they HAVE spent money or earned money. DO NOT call this for hypothetical scenarios.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      type: {
        type: Type.STRING,
        enum: ["income", "expense"],
        description: "Is this money coming in (income) or going out (expense)?",
      },
      amount: {
        type: Type.NUMBER,
      ...
      category: {
        type: Type.STRING,
        description:
          "Category like Food, Transport, Tech, Housing, Salary, Books, Leisure, etc.",
      },
      merchant: {
        type: Type.STRING,
        description: "The name of the store, person, or entity.",
      },
      date: {
        type: Type.STRING,
        description:
          'The date of the transaction in YYYY-MM-DD format. Calculate this based on "today", "yesterday", "last friday", or specific dates mentioned.',
      },
      isRecurring: {
        type: Type.BOOLEAN,
        description: "Does this happen repeatedly?",
      },
      recurringFrequency: {
        type: Type.STRING,
...
const addGoalTool: FunctionDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      name: {
        type: Type.STRING,
        description: "Name of the goal (e.g., New Car).",
      },
...
const getSystemInstruction = (context: FinancialContext): string => {
  ...
  TODAY'S DATE: ${dateString} (ISO: ${isoDate}).
  Use this date as the reference anchor to calculate dates for "yesterday", "last week", "2 days ago", etc.

  CURRENT FINANCIAL SNAPSHOT:
  - Balance: $${context.totalBalance.toFixed(2)}
  - Monthly In: $${context.monthlyIncome.toFixed(
    2
  )} | Out: $${context.monthlyExpense.toFixed(2)}
  ...
  2. **Hypothetical Mode ("What If"):** If the user asks "What if I spend...", "Can I afford...", or "Hypothetically...", do NOT call any tools. Instead, perform a budget simulation in your response. 
     ...
     - Suggest cuts if the hypothetical expense puts them over budget (e.g., "You spend a lot on ${
       context.topExpenseCategory
     }, try cutting that down").
  3. **Action Mode**: Only call 'addTransaction' or 'addGoal' if the user confirms a real action (e.g., "I just bought...", "Add a goal...").
     - **Dates**: If the user says "I spent $50 yesterday", CALCULATE the date based on TODAY'S DATE (${isoDate}) and pass it in the 'date' field of the function call.
